          ...
          {Array.from({ length: 150 }).map((_, i) => {
            ...
            return (
              <div key={i} className="h-16 md:h-24 bg-[#020202] border border-[#0a0a0a] relative shadow-lg flex items-center justify-center">
                ...
                {hasLight && (
                  <div 
                    className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-[#1fff0f]/40 rounded-full"
                    style={{ 
    ...
    <div className="fixed inset-0 z-[999] bg-black text-[#1fff0f] font-mono overflow-hidden select-none flex flex-col items-center justify-center p-8">
      ...
      <div className={`relative z-10 w-full max-w-3xl text-left h-full flex flex-col justify-center transition-all ${isGlitching ? 'hacker-glitch' : ''} ${isUnstableScene ? 'unstable-system' : ''}`}>
        ...
        {phase === 5 && (
          <>
            {renderServerRoom()}
            <div className="relative z-10 text-sm md:text-lg tracking-widest text-[#1fff0f]/80 flex flex-col gap-8 bg-black/60 p-8 border border-[#1fff0f]/20 backdrop-blur-sm">
              <div>
      ...
      {showSkip && (
        <button 
          onClick={finishIntro}
          className="absolute bottom-6 right-8 text-[#1fff0f]/40 hover:text-[#1fff0f] font-mono text-xs tracking-[0.2em] transition-colors z-[100] border border-[#1fff0f]/20 hover:border-[#1fff0f] px-3 py-1 bg-black/50"
        >
          [ENTER] SKIP
        </button>
      )}
      ...
      <style>{`
        ...
        .glow-text {
          text-shadow: 0 0 8px rgba(31, 255, 15, 0.3);
        }
        .glow-text-white {
        ...
        .server-room-grid {
          ...
          gap: 4px;
          animation: slow-zoom 8s forwards ease-in-out;
        }
        @keyframes slow-zoom {
