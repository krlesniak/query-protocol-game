...
export const EvidenceModal = ({ evidenceId, onClose }: EvidenceModalProps) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[100] flex items-center justify-center bg-[#030505]/90 backdrop-blur-md p-2 sm:p-6" onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="bg-[#090c0c] border border-[#303630] w-[95vw] max-w-7xl h-[90vh] flex flex-col shadow-[0_20px_60px_rgba(0,0,0,0.8)] overflow-hidden relative" onClick={e => e.stopPropagation()}>
        ...
        <AnimatePresence>
          ...
          {jumpscarePhase === 2 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[500] bg-[#050000] flex items-center justify-center pointer-events-none">
              <motion.div animate={{ opacity: [0.7, 1, 0.75, 1, 0.8], scale: [1, 1.02, 0.99, 1.01, 1] }} transition={{ duration: 0.3, repeat: Infinity, ease: 'linear' }} className="text-4xl sm:text-6xl font-black text-[#6b0000] tracking-[0.3em] font-mono flex items-center gap-6" style={{ textShadow: '0 0 5px #250000, 0 0 15px #500000, 0 0 35px #350000' }}>
              ...
              <motion.div initial={{ opacity: 0, y: 8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.98 }} transition={{ duration: 0.18 }} className="relative w-[90%] max-w-md bg-[#090d0f] border border-[#3a4440] shadow-[0_18px_60px_rgba(0,0,0,0.75)] overflow-hidden">
                ...
                <div className="relative px-6 pt-5">
                  <div className="flex items-center justify-between pb-4 border-b border-[#303936]">
                    ...
                    <button onClick={() => setShowPasswordPrompt(false)} className="text-[#59625f] hover:text-[#a08a52] transition-colors"><X className="w-4 h-4" /></button>
                  </div>
                </div>
                ...
                <div className="relative p-6">
                  ...
                  <form onSubmit={handlePasswordSubmit} className="flex flex-col gap-4">
                    ...
                    <div className="relative">
                      <input type="text" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} autoFocus className="w-full bg-[#040708] border border-[#343d3a] text-[#a9ae99] font-mono text-sm px-4 py-3 outline-none tracking-[0.16em] uppercase transition-all placeholder:text-[#343c39] focus:border-[#716544] focus:shadow-[inset_0_0_18px_rgba(139,116,66,0.04)]" />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-[#65735e] opacity-70" />
                    </div>
                    {passwordError && (
                      <div className="border-l-2 border-[#76584d] bg-[#160f0d] px-3 py-2 text-[#a87965] font-mono text-[9px] tracking-[0.12em]">AUTHENTICATION FAILURE — ACCESS DENIED</div>
                    )}
                    <button type="submit" className="mt-2 bg-[#111715] border border-[#454b45] text-[#9a9b87] hover:bg-[#171d1a] hover:border-[#756a4d] hover:text-[#b5aa7c] p-3 font-mono text-[10px] tracking-[0.2em] transition-all duration-200">AUTHENTICATE</button>
                  </form>
                ...
                <div className="relative border-t border-[#343b38] bg-[#070a0b] px-6 py-2.5 flex justify-between items-center">
        ...
        <AnimatePresence>
          {showSecret && ev?.easterEgg && (
            <motion.div initial={{ opacity: 0, y: 12, filter: 'blur(2px)' }} animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }} exit={{ opacity: 0, y: 12 }} transition={{ duration: 0.35, ease: 'easeOut' }} className="absolute bottom-4 sm:bottom-8 left-1/2 -translate-x-1/2 z-[200] max-w-lg w-[calc(100%-2rem)] sm:w-[90%] bg-[#050706]/95 border border-[#303630] shadow-[0_12px_40px_rgba(0,0,0,0.65)] font-mono overflow-hidden">
              <div className="h-1 w-full bg-[#a3ad82] opacity-50" />
              <div className="p-4 sm:p-6">
                <div className="flex justify-between items-start gap-4 mb-4 pb-3 border-b border-[#303630]">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 text-[#a3ad82] text-[9px] sm:text-[10px] tracking-[0.18em] font-bold">
                      <Key className="w-3 h-3 sm:w-4 sm:h-4 shrink-0" />
                      <span className="truncate">HIDDEN DATA FRAGMENT</span>
                    </div>
                    <div className="mt-1 text-[8px] sm:text-[9px] tracking-[0.16em] text-[#70756d]">NEXUS DATABASE // RECOVERY CHANNEL</div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <span className="text-[8px] sm:text-[9px] tracking-widest text-[#806d4b] border border-[#806d4b]/30 px-2 py-1">RECOVERED</span>
                    <button type="button" onClick={() => setShowSecret(false)} className="text-[#656a63] hover:text-[#c0c2b9] transition-colors"><X className="w-4 h-4" /></button>
                  </div>
                </div>
                <div className="border-l border-[#806d4b]/40 pl-3 sm:pl-4">
                  <p className="font-mono text-[11px] sm:text-sm text-[#b3b5ad] leading-relaxed tracking-wide whitespace-pre-wrap">{ev.easterEgg.message}</p>
                </div>
                <div className="mt-4 pt-3 border-t border-[#303630] flex flex-wrap justify-between gap-2 text-[8px] sm:text-[9px] tracking-[0.14em] text-[#555a53]">
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-[#303630] bg-[#070909] shrink-0">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <span className="font-mono text-[10px] sm:text-[12px] text-[#c0c2b9] tracking-[0.1em] sm:tracking-[0.2em] font-bold truncate">NEXUS_OS // CLASSIFIED DATA</span>
          </div>
          <div className="flex items-center gap-4 sm:gap-6">
            {ev?.easterEgg && (
              <span className={`hidden sm:flex font-mono text-[9px] sm:text-[10px] tracking-widest px-2 py-1 border rounded-none ${isSecretDiscovered ? 'text-[#a3ad82] border-[#4e574d] bg-[#0a0d0c]' : 'text-[#555a53] border-[#303630]'}`}>
        ...
        {/* BODY */}
        {!ev ? (
          <div className="p-10 text-center font-mono text-[#956b59] text-sm sm:text-base">ERROR: FILE CORRUPTED OR NOT FOUND</div>
        ) : (
          <div className="flex flex-col lg:flex-row h-full min-h-0 overflow-hidden">
            <div className="lg:w-[65%] xl:w-[70%] bg-[#030505] border-b lg:border-b-0 lg:border-r border-[#303630] relative flex shrink-0 overflow-hidden">
              ...
              {!imageError ? (
                    ...
                    <>
                      ...
                      <TransformComponent wrapperStyle={{ width: "100%", height: "100%" }} contentStyle={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <div className="relative w-full h-full flex items-center justify-center">
                          <img src={ev.imagePath} alt={ev.title} className="w-full h-full object-contain relative z-10 shadow-2xl border border-[#303630] cursor-grab active:cursor-grabbing" onError={() => setImageError(true)} />
                          {ev.easterEgg && (
                            <div 
                              onClick={(e) => { e.stopPropagation(); handleEasterEggClick(); }}
                              className={`absolute z-20 transition-all duration-500 cursor-crosshair overflow-hidden ${showSecret ? 'border border-[#a3ad82]/40 bg-[#a3ad82]/10' : 'hover:border hover:border-[#a3ad82] hover:bg-[#a3ad82]/20 hover:shadow-[0_0_15px_rgba(163,173,130,0.3)]'}`}
                            ...
                            >
                              {ev.easterEgg.actionType === 'redacted' && isSecretDiscovered && (
                                <div className="w-full h-full bg-[#050707] border border-[#a3ad82] text-[#a3ad82] flex items-center justify-center font-mono text-[8px] sm:text-[10px] whitespace-nowrap z-30 shadow-[0_0_10px_rgba(163,173,130,0.3)]">
                ...
                <div className="flex flex-col items-center justify-center text-[#555a53] z-10 w-full h-full border border-dashed border-[#303630] bg-black/20 backdrop-blur-sm p-4 sm:p-8">
            ...
            <div className="lg:w-[35%] xl:w-[30%] flex flex-col p-4 sm:p-8 bg-[#090c0c] lg:overflow-y-auto z-10 relative shadow-[-10px_0_30px_rgba(0,0,0,0.5)]">
              <div className="mb-6 sm:mb-8 shrink-0">
                <h2 className="text-xl sm:text-2xl lg:text-3xl font-mono text-[#c0c2b9] tracking-wide uppercase leading-tight mb-4 sm:mb-6">{ev.title}</h2>
                <div className="flex flex-wrap gap-2">
                  <span className="text-[9px] sm:text-[10px] font-mono text-[#a3ad82] px-2 py-1 border border-[#4e574d] bg-[#0a0d0c] rounded-none">TYPE: {ev.type}</span>
                  <span className="text-[9px] sm:text-[10px] font-mono text-[#806d4b] px-2 py-1 border border-[#665b43] bg-[#0c0e0d] rounded-none">SOURCE_LVL: {ev.sourceLevel.toString().padStart(2, '0')}</span>
                  <span className="text-[9px] sm:text-[10px] font-mono text-[#656a63] px-2 py-1 border border-[#303630] bg-[#050707] rounded-none">ID: {ev.id}</span>
                </div>
              </div>
              <div className="flex-1">
                <h3 className="font-mono text-[10px] sm:text-[12px] tracking-[0.2em] text-[#70756d] mb-3 sm:mb-4 border-b border-[#303630] pb-2">INVESTIGATOR'S NOTES //</h3>
                <p className="font-mono text-[12px] sm:text-[14px] text-[#b3b5ad] leading-relaxed whitespace-pre-wrap">{ev.storyDescription || ev.description}</p>
              </div>
            </div>
          </div>
        )}
